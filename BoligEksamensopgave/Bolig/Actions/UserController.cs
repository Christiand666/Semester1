...
namespace Bolig.Actions
{
    public class UserController
    {
        ...
        public static Agent GetAgentByName(string name)
        {
            ...
            Agent a;
            if (name != null)
                a = Context.Agents.Where(x => x.Name == name).FirstOrDefault();
            else
                a = null;
            return a;
        }
        ...
        public static List<Estate> GetHistoryOfEstates(int customerID)
        {
            var Context = new Entities();
            var userHist = Context.Histories.Where(X => X.CustomerID == customerID).ToList<History>();
            var AllEstates = Context.Estates.Where(X => X.ID != 0).ToList<Estate>();

            List<Estate> estateHist = new List<Estate>();
            foreach(var item in userHist)
            {
                var singleEstate = AllEstates.Where(x => x.ID == item.EstateID).FirstOrDefault();
                if (singleEstate != null)
                    estateHist.Add(singleEstate);
                if (estateHist.Count() == userHist.Count())
                    break;
            }
            return estateHist;
        }
        ...
        {
            var Context = new Entities();
            var Users = Context.Customers.Where(X => X.ID != 0).ToList<Customer>();
            if (Users == null)
                return null;
            return Users;
        }
